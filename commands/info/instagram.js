...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!name) {
            return message.reply("Может Вы напишите ник для нахождения человека, а?..")
        ...
        if (!res.graphql.user.username) {
            return message.reply("Я не могу найти такой аккаунт... Посмотрите, может ошиблись в написании?")
                .then(m => m.delete(5000));
        };
        ...
        const embed = new RichEmbed()
            ...
            .addField("Информация об аккаунте", stripIndents `**- Ник:** ${account.username}
            **- Полное имя:** ${account.full_name}
            **- Биография(описание):** ${account.biography.length == 0 ? "Пусто" : account.biography}
            ...
            **- Приватность аккаунта:** ${account.is_private ? "Да 🔐" : "Нет 🔓"}`);
